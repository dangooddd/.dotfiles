...
function oauthServers(configs: Map<string, ServerConfig>) {
...
async function loadConfigs(ctx: ExtensionContext) {
    configs.clear();
    const files = [join(getAgentDir(), "mcp.json")];

    if (ctx.isProjectTrusted()) {
        files.push(join(ctx.cwd, CONFIG_DIR_NAME, "mcp.json"));
    }
    ...
    for (const file of files) {
        ...
        try {
            config = (await readJson(file)) as McpConfig;
        } catch (e) {
            ctx.ui.notify(`Failed to load MCP config ${file}: ${errorMessage(e)}`, "warning");
            continue;
        }
...
async function connectAllServers(
    ...
    const results = await mapLimit([...configs], CONNECT_CONCURRENCY, async ([name, config]) => {
        try {
...
export default function mcp(pi: ExtensionAPI) {
    ...
    pi.registerCommand("mcp-auth", {
        ...
        handler: async (_args, ctx) => {
            const oauth = oauthServers(configs);
